...
void init_state_cache( Data const& data, Posteriors const& post, State& state )
{
    ...
    state.x_col_sum_squared.resize( data.num_snps, 0.0 );
    state.product_x_beta.resize( data.num_indiv, 0.0 );
    ...
    for( size_t s = 0; s < data.num_snps; ++s ) {
        for( size_t i = 0; i < data.num_indiv; ++i ) {
            state.x_col_sum_squared[s] += data.x(i, s) * data.x(i, s);
        }
    }
    ...
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        for( size_t s = 0; s < data.num_snps; ++s ) {
            state.product_x_beta[i] += data.x(i, s) * post.beta[s];
        }
    }
...
{
    assert( data.num_indiv == state.product_x_beta.size() );
    assert( data.num_indiv == state.product_c_alpha.size() );

    // Compute sum of squared residuals
    double sum_square_resid = 0.0;
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        auto const resid = data.y[i] - state.product_x_beta[i] - state.product_c_alpha[i];
...
void sample_and_update_alpha( Data const& data, Posteriors& post, State& state )
{
    ...
    if( data.num_covar == 1 ) {
        auto const c = 0;
        for( size_t i = 0; i < data.num_indiv; ++i ) {
            auto const y_x_beta = data.y[i] - state.product_x_beta[i];
...
void sample_and_update_gamma( Data const& data, Posteriors& post, State& state )
{
    ...
    for( size_t s = 0; s < data.num_snps; ++s ) {
        ...
        for( size_t i = 0; i < data.num_indiv; ++i ) {
             auto const residual = data.y[i] - state.product_c_alpha[i] - state.product_x_beta[i] + data.x(i,s) * post.beta[s];
             dot_prod_residuals += residual * data.x(i, s);
        }

        auto const variance = 1.0 / ( state.x_col_sum_squared[s] * sigma_e_neg2 + sigma_1_neg2 );
        auto const mean = variance * dot_prod_residuals * sigma_e_neg2;

        // Compute the intermediate values
        auto const d = state.x_col_sum_squared[s] * sigma_e_neg2 / sigma_1_neg2 + 1.0;
...
void sample_and_update_beta(
    ...
    for( size_t s = 0; s < data.num_snps; ++s ) {
        ...
        if( post.gamma[s] == 0 ) {
            // if gamma[s] ==0. then state.product_x_beta doesnt need adjustment
            post.beta[s] = 0.0;
        } else{
            ...
            for( size_t i = 0; i < data.num_indiv; ++i ) {
                ...
                auto const residual = data.y[i] - state.product_c_alpha[i] - state.product_x_beta[i];
                dot_prod_residuals += residual * data.x(i,s);
            }
            auto const variance = 1.0 / ( state.x_col_sum_squared[s]  * sigma_e_neg2 + sigma_1_neg2 );
            auto const mean = variance * dot_prod_residuals * sigma_e_neg2;
...
Statistics compute_sample_statistics(
    ...
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        geno_moments.push( state.product_x_beta[i] );
