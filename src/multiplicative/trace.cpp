...
void MultiplicativeTrace::push_to_trace(
    size_t iteration,
    ...
    MultiplicativeTrace::Entry entry;
...
void MultiplicativeTrace::pop_from_trace( size_t num_elements )
{
    num_elements = std::min( num_elements, entries.size() );
    entries.erase( entries.begin(), entries.begin() + num_elements );
}

void MultiplicativeTrace::summarize_trace( Data const& data, std::string const& file_prefix ) const
{
...
void MultiplicativeTrace::write_trace( std::string const& file_prefix ) const
{
    using namespace genesis::utils;
