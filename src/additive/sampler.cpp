...
void AdditiveSampler::init_posteriors()
{
...
void AdditiveSampler::init_cache()
{
    // Init the caches.
    cache.c_col_sum_squared.resize( data.num_covar, 0.0 );
    cache.product_c_alpha.resize( data.num_indiv, 0.0 );
    cache.x_col_sum_squared.resize( data.num_snps, 0.0 );
    cache.product_x_beta.resize( data.num_indiv, 0.0 );
    ...
    // pre-compute the per-column sum of squares of x
    for( size_t s = 0; s < data.num_snps; ++s ) {
        for( size_t i = 0; i < data.num_indiv; ++i ) {
            cache.x_col_sum_squared[s] += data.x(i, s) * data.x(i, s);
        }
    }

    // compute the x_beta (matrix_multiplication)
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        for( size_t s = 0; s < data.num_snps; ++s ) {
            cache.product_x_beta[i] += data.x(i, s) * post.beta[s];
        }
    }
...
double AdditiveSampler::sample_pi()
{
...
double AdditiveSampler::sample_sigma_1()
{
    // Compute new a and b
...
double AdditiveSampler::sample_sigma_e()
{
    assert( data.y.cols() == 1 );
    assert( data.num_indiv == data.y.rows() );
    assert( data.num_indiv == cache.product_x_beta.size() );
    assert( data.num_indiv == cache.product_c_alpha.size() );

    // Compute sum of squared residuals
    double sum_square_resid = 0.0;
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        auto const resid = data.y( i, 0 ) - cache.product_x_beta[i] - cache.product_c_alpha[i];
...
void AdditiveSampler::sample_and_update_alpha()
{
    assert( data.num_indiv == data.y.rows() );
    assert( data.num_indiv == cache.product_x_beta.size() );
    assert( data.num_indiv == cache.product_c_alpha.size() );
    ...
    if( data.num_covar == 1 ) {
        auto const c = 0;
        for( size_t i = 0; i < data.num_indiv; ++i ) {
            auto const y_x_beta = data.y( i, 0 ) - cache.product_x_beta[i];
        ...
        for( size_t c = 0; c < data.num_covar; ++c ) {
            for( size_t i = 0; i < data.num_indiv; ++i ) {
                ...
                auto const y_negi = data.y( i, 0 ) - c_alpha_negc_i - cache.product_x_beta[i];
...
void AdditiveSampler::sample_and_update_gamma()
{
    ...
    for( size_t s = 0; s < data.num_snps; ++s ) {
        ...
        // compute the mean and variance for snp[s]
        double dot_prod_residuals = 0.0;

        for( size_t i = 0; i < data.num_indiv; ++i ) {
             auto const residual = data.y(i, 0) - cache.product_c_alpha[i] - cache.product_x_beta[i] + data.x(i,s) * post.beta[s];
             dot_prod_residuals += residual * data.x(i, s);
        }

        auto const variance = 1.0 / ( cache.x_col_sum_squared[s] * sigma_e_neg2 + sigma_1_neg2 );
        auto const mean = variance * dot_prod_residuals * sigma_e_neg2;

        // Compute the intermediate values
        auto const d = cache.x_col_sum_squared[s] * sigma_e_neg2 / sigma_1_neg2 + 1.0;
...
void AdditiveSampler::sample_and_update_beta()
{
    ...
    for( size_t s = 0; s < data.num_snps; ++s ) {
        // first remove the sth beta from the x_beta product
        for( size_t i = 0; i < data.num_indiv; ++i ) {
            cache.product_x_beta[i] -= data.x(i, s) * post.beta[s];
        }

        if( post.gamma[s] == 0 ) {
            // if gamma[s] ==0. then cache.product_x_beta doesnt need adjustment
            post.beta[s] = 0.0;
        } else{
            ...
            for( size_t i = 0; i < data.num_indiv; ++i ) {
                ...
                auto const residual = data.y(i, 0) - cache.product_c_alpha[i] - cache.product_x_beta[i];
                dot_prod_residuals += residual * data.x(i,s);
            }
            auto const variance = 1.0 / ( cache.x_col_sum_squared[s]  * sigma_e_neg2 + sigma_1_neg2 );
            auto const mean = variance * dot_prod_residuals * sigma_e_neg2;
            ...
            if( std::abs( post.beta[s] ) < hyper.min_beta ) {
                // if the updated beta is 0, then the product_x_beta doesnt need adjustment
                post.beta[s] = 0.0;
            } else{
                for( size_t i = 0; i < data.num_indiv; ++i ) {
                    cache.product_x_beta[i] += data.x(i, s) * post.beta[s];
...
void AdditiveSampler::sampling_step()
{
...
bool AdditiveSampler::has_converged()
{
    ...
    auto update_zscore_with_trace_variable = [&](
        AdditiveHyperparams const& hyper, AdditiveTrace const& trace,
        std::function<double(AdditiveTrace::Entry const&)> select_value,
        double& max_zscore
    ) {
    ...
    for( size_t c = 0; c < data.num_covar; ++c ) {
        update_zscore_with_trace_variable(
            hyper, trace,
            [&]( AdditiveTrace::Entry const& entry ){
...
AdditiveStatistics AdditiveSampler::compute_sample_statistics()
{
    assert( data.num_indiv == data.x.rows() );
    assert( data.num_indiv == data.y.rows() );
    assert( data.num_indiv == cache.product_x_beta.size() );
    ...
    for( size_t i = 0; i < data.num_indiv; ++i ) {
        geno_moments.push( cache.product_x_beta[i] );
        pheno_moments.push( data.y( i, 0 ) - cache.product_c_alpha[i] );

        // Compute large beta x product for the current individual for its moment
        double large_beta_prod = 0;
        for( size_t s = 0; s < data.num_snps; ++s ) {
            if( std::abs( post.beta[s] ) > hyper.large_beta ) {
                large_beta_prod += data.x(i, s) * post.beta[s];
            }
        }
        large_beta_moments.push( large_beta_prod );
    }
...
bool AdditiveSampler::valid_sample()
{
